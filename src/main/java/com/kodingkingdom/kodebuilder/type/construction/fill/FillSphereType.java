...
public class FillSphereType extends AsyncType{
	...
	private static final Pattern fillspherepattern = Pattern.compile("fill(.+)(?:sphere)(.+)");
	private static final Pattern fillellipsoidpattern = Pattern.compile("fill(.+)(?:sphere)(north|south|east|west|up|down)(.+)(north|south|east|west|up|down)(.+)(north|south|east|west|up|down)(.+)");
	@Override
	public FillSphereBlock makeKodeBlock(String kodeString) {
		Matcher spherematcher = fillspherepattern.matcher(kodeString.replaceAll("\\s", "").toLowerCase());
		Matcher ellipsoidmatcher = fillellipsoidpattern.matcher(kodeString.replaceAll("\\s", "").toLowerCase());
		if (ellipsoidmatcher.matches()){
			...
			return new FillSphereBlock(this,kodeString,
					slotExpr,
	...
	private class FillSphereBlock extends KodeBlock{
		...
		public KodeBlock clone() {
			return new FillSphereBlock((FillSphereType)blockType,kodeString,slotExpr,directionOne,directionTwo,directionThr,radiusOneExpr,radiusTwoExpr,radiusThrExpr);}
		private FillSphereBlock(FillSphereType Type, String Code, Parser<Expression> SlotExpr,  BlockFace DirectionOne, BlockFace DirectionTwo, BlockFace DirectionThr, Parser<Expression> RadiusOneExpr, Parser<Expression> RadiusTwoExpr, Parser<Expression> RadiusThrExpr){
	...
	protected void preprocess(KodeSchedule sch, KodeDroid droid, KodeBlock blk) throws KodeException {
		...
		if (!(blk instanceof FillSphereBlock)) throw new RuntimeException();
		FillSphereBlock b = (FillSphereBlock)blk;
		try{
			this.droid=droid;
